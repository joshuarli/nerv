...
/// Locate the `llama-gguf` binary on $PATH.
pub fn find_llama_gguf() -> Option<PathBuf> {
    which("llama-gguf")
}
...
/// Write a llama-gguf TOML config for `model` to `config_path` and return
/// the `serve` CLI args: `["serve", "--config", "<path>", "--host", "...", "--port", "...", "<model_path>"]`
///
/// The TOML contains smart defaults:
///   - `model.gpu = true` when the model fits in available memory
///   - `model.kv_cache_type = "turboquant2-qjl"` for reduced VRAM usage
///   - Generous context window derived from available RAM
///   - Sampling params tuned for agentic use (low temp, high top-p)
pub fn write_llama_gguf_config(
    model: &LocalModel,
    config_path: &Path,
) -> anyhow::Result<Vec<String>> {
    let hw = detect_hardware();
    let model_path = model.resolved_path();
    let model_size_bytes = std::fs::metadata(&model_path).map(|m| m.len()).unwrap_or(0);
    let model_size_gb = model_size_bytes as f64 / (1024.0 * 1024.0 * 1024.0);

    // Reserve 4 GB for OS + nerv itself
    let available_gb = (hw.total_memory_gb - 4.0).max(2.0);
    let gpu = model_size_gb < available_gb;

    // Context window — same calculation as recommended_defaults()
    let est_params_b = model_size_gb / 0.55;
    let kv_per_1k_ctx_mb = est_params_b * 0.25;
    let remaining_gb = (available_gb - model_size_gb).max(0.5);
    let max_ctx_from_memory = ((remaining_gb * 1024.0) / kv_per_1k_ctx_mb * 1024.0) as u32;
    let ctx_len = max_ctx_from_memory.clamp(4096, 131_072);
    let ctx_len = (ctx_len / 1024) * 1024;

    // KV cache type: turboquant2-qjl gives ~4× memory savings vs f32
    // with good quality on Apple Silicon unified memory and CUDA.
    let kv_cache_type = "turboquant2-qjl";

    // Sampling: for coding/agentic tasks prefer low temperature / high top-p
    let max_tokens: u32 = if remaining_gb > 6.0 { 8192 } else { 4096 };

    let toml = format!(
        "# llama-gguf configuration generated by nerv\n\
         # Re-generated on each `nerv load` — edit ~/.nerv/models.json to change settings.\n\
         \n\
         [model]\n\
         gpu = {gpu}\n\
